...
class SoftwareInstall(object):
    ...
    def __init__(self, argumentsDict):
        ...
        if 'SoftwarePackages' in self.job:
            if isinstance(self.job['SoftwarePackages'], str):
    ...
    def execute(self):
        ...
        for app in self.apps:
            ...
            if not os.path.exists("%s/%s/%s" % (os.environ["HOME"], name, version)):
                ...
                gLogger.info("Added %s_%s_DIR to the os.environ:" % (name, version),
                             os.environ["%s_%s_DIR" % (name, version)])
            else:
            ...
            with open(fpath, "w") as script:
                ...
                comm = 'rsync -avz -e "ssh -i %(home)s/.ssh/id_dsa -p %(port)s" %(user)s@%(host)s:%(path)s/Packages/%(name)s/%(version)s %(home)s/%(name)s/\n' \
                       % {"home": os.environ["HOME"], "user": self.user, "host": self.host, "port": self.port,
        ...
        for dep in deps_list:
            ...
            with open(fpath, "w") as script:
                ...
                comm = 'rsync -avz -e "ssh -i %(home)s/.ssh/id_dsa -p %(port)s" %(user)s@%(host)s:%(path)s/Packages/%(package)s %(home)s/Packages/\n' \
                       % {"home": os.environ["HOME"], "port": self.port, "host": self.host, "user": self.user,
                          "path": self.path, "package": dep['name']}
                gLogger.info("Rsync %s with" % dep["name"], comm)
                script.write(comm)
                script.write("exit $?\n")
            os.chmod(fpath, 0755)
            # try to rsync Packages
            # assuming the key is located in $HOME/.ssh/id_dsa
            comm = ["sh", '-c', fpath]
            try:
                subprocess.check_call(comm)
            except subprocess.CalledProcessError:
